...
def autoclick():
    ...
    if repeatVar.get()==1:
        ...
        autoclickButton.configure(bg="gray")
        return
Delays = LabelFrame(window, width = 450, height = 45, pady=10, padx=10, text="Delays")
Delays.pack()
...
Controls = LabelFrame(window, width = 450, height = 45, pady=10, padx=10, text="Controls")
Controls.pack()
...
alwaysOnTopVar=IntVar()
alwaysOnTop = tkinter.Checkbutton(Controls, text="Always on top", variable=alwaysOnTopVar, command=lambda : onTop())
alwaysOnTop.grid(row=0, column=3, padx=5, pady=5)
def onTop():
    if alwaysOnTopVar.get()==1:
        window.wm_attributes("-topmost", 1)
    else:
        window.wm_attributes("-topmost", 0)
Repeats = LabelFrame(window, width = 450, height = 45, pady=10, padx=10, text="Repeats")
Repeats.pack()
